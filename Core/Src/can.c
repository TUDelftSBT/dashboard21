...
uint8_t can_msg_rx_hydrogenAlarm2_data[8];
uint8_t rr_hydrogenAlarm2_handle = 0;
uint8_t rr_hydrogenAlarm2_timeout = 0;
uint32_t HydrogenTimeLastMessage2 = 0;
...
void CAN1ReceiveMsg(uint32_t count)
{
	...
	if (CAN1RxHeader.StdId == can_msg_rx_hydrogenAlarm2_id)	//if the message in the buffer is from the hydrogen alarm store it in the proper list
	{
		can_msg_rx_hydrogenAlarm2_data[0] = CAN1RxData[0];
		can_msg_rx_hydrogenAlarm2_data[1] = CAN1RxData[1];
		can_msg_rx_hydrogenAlarm2_data[2] = CAN1RxData[2];
		can_msg_rx_hydrogenAlarm2_data[3] = CAN1RxData[3];
		can_msg_rx_hydrogenAlarm2_data[4] = CAN1RxData[4];
		can_msg_rx_hydrogenAlarm2_data[5] = CAN1RxData[5];
		can_msg_rx_hydrogenAlarm2_data[6] = CAN1RxData[6];
		can_msg_rx_hydrogenAlarm2_data[7] = CAN1RxData[7];
